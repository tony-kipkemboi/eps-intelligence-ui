...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
	const { ssl, max, user, host, port, database, parsers: parsers$1, transform, onnotice, onnotify, onparameter, max_pipeline, keep_alive, backoff: backoff$1, target_session_attrs } = options;
	...
	function ErrorResponse(x) {
		query && (query.cursorFn || query.describeFirst) && write(Sync);
		const error$1 = Errors.postgres(parseError(x));
		query && query.retried ? errored(query.retried) : query && query.prepared && retryRoutines.has(error$1.routine) ? retry(query, error$1) : errored(error$1);
	}
